...
jsPsych.plugins['survey-multi-choicemr'] = (function() {

  var plugin = {};

  plugin.trial = function(display_element, trial) {

    var plugin_id_name = "jspsych-survey-multi-choicemr";
    ...
    // add multiple-choicemr questions
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      $(question_selector).append(
        '<p class="' + plugin_id_name + '-text survey-multi-choicemr">' + trial.questions[i] + '</p>'
      );
    ...
    $trial_form.submit(function(event) {
      ...
      $("div." + plugin_id_name + "-question").each(function(index) {
        var id = "Q" + index;
        var val = $(this).find("input:checkbox:checked").val();
