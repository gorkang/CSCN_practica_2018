  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        html += '<input type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns+'" value="'+trial.questions[i].value+'" autofocus></input> '+trial.endword;
      } else {
        html += '<input type="text" name="#jspsych-survey-text-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '" autofocus></input> '+trial.endword;
      }
      html += '</div>';
    }
    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function() {
      ...
      //if ((validation >= 12 && validation <= 14) || (validation >= 21 && validation <= 24) || (validation >= 31 && validation <= 34) || (validation >= 41 && validation <= 43) &&  validation != 22 && validation != 33 ) {
            ...
            firstTextBox.blur();
            display_element.querySelector(".fail-message").innerHTML = '<span style="color: red;" class="required">Por favor inserte texto valido.</span>';
