/* jspsych-instructionsbias.js
 * Josh de Leeuw
 *
 * This plugin displays text (including HTML formatted strings) during the experiment.
 * Use it to show instructionsbias, provide performance feedback, etc...
 *
...
jsPsych.plugins.instructionsbias = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function show_current_page() {
      ...
      if (trial.show_clickable_nav) {
        ...
        var nav_html = "<div class='jspsych-instructionsbias-nav'>";
        if (current_page != 0 && trial.allow_backward) {
          nav_html += "<button id='jspsych-instructionsbias-back' class='jspsych-btnbias'>&lt; Volver</button>";
        }
        nav_html += "<button id='jspsych-instructionsbias-next' class='jspsych-btnbias'>Siguiente &gt;</button></div>"

        display_element.append(nav_html);

        if (current_page != 0 && trial.allow_backward) {
          $('#jspsych-instructionsbias-back').on('click', function() {
            clear_button_handlers();
            back();
          });
        }

        $('#jspsych-instructionsbias-next').on('click', function() {
          clear_button_handlers();
          next();
        });
    ...
    function clear_button_handlers() {
      $('#jspsych-instructionsbias-next').off('click');
      $('#jspsych-instructionsbias-back').off('click');
    }
