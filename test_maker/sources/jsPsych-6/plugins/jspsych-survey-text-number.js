...
jsPsych.plugins['survey-text-number'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    display_element.querySelector('#jspsych-survey-text-number-next').addEventListener('click', function() {
      ...
      // next trial
      if ($.isNumeric(obje[id]) === true) {
        display_element.innerHTML = '';
        jsPsych.pluginAPI.clearAllTimeouts();
        jsPsych.finishTrial(trialdata);
      }else{
        firstTextBox.blur();
        display_element.querySelector(".fail-message").innerHTML = '<span style="color: red;" class="required">Por favor ingresa una número válido.</span>';
      }
    });
