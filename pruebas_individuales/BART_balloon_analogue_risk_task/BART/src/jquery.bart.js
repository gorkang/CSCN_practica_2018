...
(function($) {
    ...
    $.fn.bart = function(method) {
        ...
        if ($.type(args) == 'number' | $.type(args) == 'array') args = {
            b: args
        };
        if ($.type(args.b) == 'number') { // number given
            ...
            for (var i = 0; i < args.b.length; i++) {
                if ($.type(args.b[i]) == 'number') args.b[i] = {
                    b: args.b[i]
                };
        ...
        balloon.prototype.inflate = function(canvas) {
            ...
            var grad = canvas.createGradient({
                x1: centerX + (this.radius / this.gradient_radius),
        ...
        balloon.prototype.save = function() {
            ...
            $('#' + opts.frmids_pumps[this.id - 1]).attr({
                value: this.pumps
            });
            $('#' + opts.frmids_exploded[this.id - 1]).attr({
                value: (this.exploded) * 1
            });
            for (var i = 1, t = 0; i < this.time.length; i++) t += this.time[i] - this.time[i - 1];
            if (this.pumps > 1) t = Math.round(t / (this.pumps - 1));
            else t = -9;
            $('#' + opts.frmids_time[this.id - 1]).attr({
                value: t
            });
                ...
                .on('click.bart', function(e) {
                    ...
                    if (opts.showpumpcount) {
                        canvas.setLayer('pumpnum', {
                            text: opts.txt_number_of_pumps + bal.pumps
                        });
                    }
                    if (opts.showcurrentearned) {
                        canvas.setLayer('curearn', {
                            text: opts.txt_current_earned +
