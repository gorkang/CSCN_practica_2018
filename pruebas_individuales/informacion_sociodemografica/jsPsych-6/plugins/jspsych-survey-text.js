  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if (trial.questions[i].rows == 1) {
        html += '<input type="text" name="#jspsych-survey-text-response-' + i + '" size="' + trial.questions[i].columns + '" value="' + trial.questions[i].value + '" autofocus onfocus="advance(event)"></input> ' + trial.endword;
      } else {
        html += '<input type="text" name="#jspsych-survey-text-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '" autofocus onfocus="advance(event)"></input> ' + trial.endword;
      }
      html += '</div>';
    }
    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function() {
      ...
      // next trial
      if (!$.isEmptyObject(validation) === true) {
        ...
        firstTextBox.blur();
        display_element.querySelector(".fail-message").innerHTML = '<span style="color: red;" class="required">Por favor ingresa texto valido.</span>';
