...
jsPsych.plugins['survey-text-number'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if (trial.preamble !== null) {
      html += '<br /><p><br /><div id="jspsych-survey-text-number-preamble" class="jspsych-survey-text-number-preamble">' + trial.preamble + '</div>';
    }
    // add questions
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if (trial.questions[i].rows == 1) {
        html += '<input type="number" min="0" name="#jspsych-survey-text-number-response-' + i + '" size="' + trial.questions[i].columns + '" value="' + trial.questions[i].value + '" autofocus></input> ' + trial.endword;
      } else {
        html += '<input type="number" min="0" name="#jspsych-survey-text-number-response-' + i + '" cols="' + trial.questions[i].columns + '" rows="' + trial.questions[i].rows + '" autofocus></input> ' + trial.endword;
      }
      html += '</div>';
    }
    ...
    var firstTextBox = document.getElementsByName("#jspsych-survey-text-number-response-0")[0];
    firstTextBox.focus();
    ...
    display_element.querySelector('#jspsych-survey-text-number-next').addEventListener('click', function() {
      ...
      // next trial
      if ($.isNumeric(validation) === true && validation > 0) {
