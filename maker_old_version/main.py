		...
		while (i < len(questions)):
			...
			if ("stimulus" in questions[i] and "{" in questions[i]["stimulus"]):
				...
				for variable in stimulus_variables:
					if variable.split(":")[0] == "variable":
						questions[i]["stimulus"] = questions[i]["stimulus"].replace("{"+ variable +"}", "' + variables['" + variable.split(":")[1] + "'] + '")
						text_modify = True
					elif variable.split(":")[0] == "image":
						questions[i]["stimulus"] = questions[i]["stimulus"].replace("{"+ variable +"}", '<img src="images/' + variable.split(":")[1] + '" />')
			...
			if questions[i]["loop"] != None:
				if loops != None and questions[i]["loop"] in loops:
					total_loops += 1
					questions_experiment[actual_int]["questions"].append({"id": "loop"+ ("{:0"+str(len(str(abs(len(questions)))))+"d}").format(total_loops)})
				else:
					print("Error, loop " + questions[i]["loop"] + " no encontrado en la lista de loops")
			...
			try:
				...
				if ( type(actual_instruction["instruction"]) != str and type(actual_instruction["instruction"]) != list ):
					...
					for index in len(actual_instruction["instruction"]):
						actual_instruction["instruction"][index] = str(actual_instruction["instruction"][index])
			except:
				pass
			try:
